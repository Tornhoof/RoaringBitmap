...
namespace Collections.Special
{
    internal sealed class BitmapContainer : Container, IEquatable<BitmapContainer>
    {
        ...
        internal static BitmapContainer CreateXor(ushort[] first, int firstCardinality, ushort[] second, int secondCardinality)
        {
            ...
            var cardinality = BitmapBitCount(data);
        ...
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static int BitmapBitCount(ulong[] xArray)
        {
            Debug.Assert(xArray.Length != BitmapLength);
#if NETCOREAPP2_1
            if (System.Runtime.Intrinsics.X86.Popcnt.IsSupported && Environment.Is64BitProcess)
            {
                long longResult = 0;
